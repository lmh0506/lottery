	...
	var Base = function () {
	  ...
	  _createClass(Base, [{
	    ...
	    value: function assistHandle(e) {
	      ...
	      var $cur = (0, _jquery2.default)(e.currentTarget);
	      var index = $cur.index();
	      var $btn = (0, _jquery2.default)('.boll-list .btn-boll');
	      $btn.removeClass('btn-boll-active');
	      if (index === 0) {
	        $btn.addClass('btn-boll-active');
	      }
	      if (index === 1) {
	        $btn.each(function (i, t) {
	    ...
	    value: function getCount() {
	      ...
	      if (count === 0) {
	        ...
	        tpl = '\n        \u60A8\u9009\u4E86<b>' + count + '</b> \u6CE8\uFF0C\u5171 <b>' + count * 2 + '</b>\u5143  <em>\u82E5\u4E2D\u5956\uFF0C \u5956\u91D1\uFF1A<strong class="red">' + range[0] + '</strong> \u5143\uFF0C\u60A8\u5C06' + (win1 >= 0 ? '盈利' : '亏损') + ' <strong class="' + (win1 >= 0 ? 'red' : 'green') + '">' + Math.abs(win1) + '</strong> \u5143</em>\n      ';
	      } else {
	        tpl = '\n        \u60A8\u9009\u4E86<b>' + count + '</b> \u6CE8\uFF0C\u5171 <b>' + count * 2 + '</b>\u5143  <em>\u82E5\u4E2D\u5956\uFF0C \u5956\u91D1\uFF1A<strong class="red">' + range[0] + '</strong> \u5143 \u81F3 <strong class="red">' + range[1] + '</strong> \u5143\uFF0C\u60A8\u5C06' + (win1 < 0 && win2 < 0 ? '亏损' : '盈利') + ' <strong class="' + (win1 >= 0 ? 'red' : 'green') + '">' + c1 + '</strong> \u5143 \u81F3 <strong class="' + (win2 >= 0 ? 'red' : 'green') + '">' + c1 + '</strong> \u5143</em>\n      ';
	      }
	    ...
	    value: function getTotal() {
	      var count = 0;
	      (0, _jquery2.default)('.codelist li').each(function (i, item) {
	  ...
	  _createClass(Calculate, [{
	    ...
	    */
	    value: function computeCount(active, play_name) {
	      ...
	      var arr = new Array(active).fill('0'); // 定义一个长度为active 元素全为0的数组
	    ...
	    value: function computeBonus(active, play_name) {
	      ...
	      var min = void 0,
	          max = void 0;
	      if (play[0] === 'r') {
	        var min_actice = 5 - (11 - active); // 最小命中数
	        if (min_actice > 0) {
	...
	var Timer = function () {
	  ...
	  _createClass(Timer, [{
	    ...
	    value: function countdown(end, update, handle) {
	      ...
	      if (now - end > 0) {
	        // 如果当前时间大于结束时间  说明倒计时结束
	        handle.call(self); // 调用handle函数 并改变它this指向
	      } else {
	        var last_time = end - now; // 当前时间离结束时间的剩余时间
	        ...
	        var h = Math.floor((last_time - d * px_d) / px_h); // 剩余多少小时
	        var m = Math.floor((last_time - d * px_d - h * px_h) / px_m); // 剩余多少分
	        var s = Math.floor((last_time - d * px_d - h * px_h - m * px_m) / px_s); // 剩余多少秒
	        var r = [];
